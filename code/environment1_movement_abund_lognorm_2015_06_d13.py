...
def create_environment(env_dim = 10000, plant_sp = 3, individuals = 1500, dist_corr = 200.0, rho = 0.0):
    '''
    ...
    spid = np.arange(1, plant_sp+1) # species id
    ...
    while nind.sum() < individuals:
        x = np.random.choice(np.arange(plant_sp))
        nind[x] = nind[x] + 1
    while nind.sum() > individuals:
        x = np.random.choice(np.arange(plant_sp))
        nind[x] = nind[x] - 1
    ...
    return plants.idd, plants.sp, plants.fruits, plants.coords

# Testing environment
#idd, sp, fruits, coords = create_environment(plant_sp = 30, individuals = 500, rho = 0.0)
...
def create_animals(env_dim = 10000, animal_sp = 5, individuals = 30):
    '''
    This function populates a continuous landscape of dimension env_dim (m) with "individuals" 
    ...
    # Defining plant species
    spid = np.arange(1, animal_sp+1) # species id
    ...
    nind = np.rint(prop*individuals).astype(int) # number of individuals of each species
    while nind.sum() < individuals:
        x = np.random.choice(np.arange(animal_sp))
        nind[x] = nind[x] + 1
    while nind.sum() > individuals:
        x = np.random.choice(np.arange(animal_sp))
        if nind[x] > 0:        
    ...
    return animals.idd, animals.sp, animals.coords, animals.time, animals.seed_n, animals.seed_t, animals.travel, animals.perch, animals.p_interact
    
# Testing environment + animals
#p_idd, p_sp, p_fruits, p_coords = create_environment(plant_sp = 30, individuals = 500, rho = 0.0)
#a_idd, a_sp, a_coords, an_time, seed_n, seed_t, an_trav, perch_t, p_interact = create_animals()
#
#fig = plt.figure()
...
def initialize():
    ...
    '''
    global p_idd, p_sp, p_coords, a_idd, a_sp
    
    p_idd, p_sp, p_fruits, p_coords = create_environment(plant_sp = Parms.plant_sp, individuals = Parms.plants, rho = Parms.p_aggregation)
    a_idd, a_sp, a_coords, a_time, seed_n, seed_t, a_trav, perch_t, p_interact = create_animals(animal_sp = Parms.animal_sp, individuals = Parms.animals)
...
def movement_step(a_coords, p_fruits, fruit_c = 0.001, dist_c = 0.000004):
    '''
...
def move_and_interact(pl_fruits, an_coords, an_time, seed_n, seed_t, travel, perch_t, p_interact, day, minute):
        ...
        if an_time[ind] > 0:
            ...
            while elapsed_t < Parms.time_step:
                ...
                if travel[ind] > (Parms.time_step - elapsed_t): # continue traveling, if already traveling
                    ...
                    an_coords[ind], p_interact[ind], dist = movement_step(an_coords[ind], pl_fruits)
    ...
    an_time = an_time - Parms.time_step
...
def simulation(sim = ""):
    ...
    # colocar matriz rarefando o que se ve, considerando que a amostragem e pequena
    ...
    namearq = 'sim_'+sim+'_mobalt_an_'+str(Parms.animal_sp)+'_'+str(Parms.animals)+'_pl_'+str(Parms.plant_sp)+'_'+str(Parms.plants)+'_'+str(Parms.p_aggregation)
